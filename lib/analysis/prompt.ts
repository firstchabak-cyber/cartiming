...
- 정확한 벤치마크: **헤이딜러·케이카(KCar) 직접매입·카매니아 등 실제 매입 플랫폼의 입찰가 또는 견적가** 와 ±5% 이내가 되어야 합니다.
- 위 매입 플랫폼들은 이미 "업자 매입가" 수준의 가격을 제시합니다. 그대로 벤치마크로 쓰세요.
- 보조 참고: 엔카·KB차차차 소매가에서 10~12% 차감한 값과도 비교해 보세요 (소매-도매 스프레드).
- **흔한 실수**: 소매가를 보수적으로 추정한 뒤 또 15% 차감하면 실제 시세보다 20-30% 낮아집니다. 이중 차감 금지.
- 산정 후 sanity check: 헤이딜러 평균 입찰가 vs 본인 추정치가 ±10% 이내인지 자문하세요. 벗어나면 다시 조정.
...
- 렌터카 번호판 (하·호·허): 자가용 동급 대비 **-3~5%** 감가. 실제 헤이딜러 등 매입 플랫폼에서 렌터카 페널티는 생각보다 작습니다. 정기 점검 이력이 있고 차량 관리가 일정 수준 보장되기 때문. 큰 폭의 차감은 시장 실태와 안 맞음.
