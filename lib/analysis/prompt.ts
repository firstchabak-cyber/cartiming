...
- 가장 정확한 벤치마크 우선순위 (위에서부터 적용):
  ...
  3. **카타임 누적 실거래가** (같은 분류: 자가용↔자가용 / 렌터카↔렌터카)
  4. **헤이딜러·케이카·엔카 평균 매입가 시장 지식** ±5% 이내
- 위 1·2·3 데이터가 있으면 **그것이 시장 지식보다 우선** 합니다. 모델 추정치가 데이터와 크게 어긋나면 데이터에 맞추세요.
- ⚠️ **앵커 보정 필수 (역전 방지 핵심)**: 외부 견적·실거래가를 기준으로 삼을 때, 그 견적이 매겨진 차량의 **주행거리·연식·시점**이 지금 차량과 다르면 **그 차이만큼 반드시 보정**하세요. 견적값을 그대로 베끼면 안 됩니다.
  · 주행거리 보정: 기준이 된 견적 차량보다 이 차가 **1만 km 더 탔으면 -1.5~2%, 덜 탔으면 +1.5~2%** 를 견적값에 가감.
  · 시점 보정: 견적 일자가 1개월 이상 지났으면 그 사이 자연 감가(월 약 -0.7~1%)를 반영.
  · 예) 헤이딜러 견적이 "2만 km 차량 3,000만원"인데 이 차가 4만 km라면 → 약 -3~4% 보정해 2,880~2,910만원으로 산정.
...
4. **주행거리 단조성(역전 금지) 체크 ⚠️**: 같은 모델·동급에서 **주행거리가 더 적고 연식이 같거나 더 최신인 차는, 더 많이 탄 차보다 절대 싸면 안 됩니다.** 만약 내 추정치가 그런 역전을 만들고 있다면, (a) 사고·교환 이력, (b) 영업용 번호판, (c) 비선호 색상 같은 **명확한 감가 사유가 실제로 있을 때만** 허용하고, 사유가 없으면 주행거리 적은 차가 더 비싸지도록 다시 조정하세요. 외부 견적 앵커가 같다는 이유로 주행거리 차이를 0으로 만들지 마세요 — 주행거리는 가장 큰 변수입니다.
...
- **신차가격(출고가)**: ${
    vehicle.msrp != null
      ? `${vehicle.msrp.toLocaleString("ko-KR")}원 — ⭐ 차주가 입력한 실제 출고가. 이 값을 감가의 출발점으로 사용하고, 여기서 차령·주행거리·사고 등을 차감해 현재가를 산정하세요.`
      : `정보 없음 → 해당 제조사·모델·연식·트림의 **신차 출고가를 먼저 추정**한 뒤, 그 추정 신차가격을 rationale [시세 평가] 단락에 "신차가 약 OOO만원 기준"으로 명시하고 감가 출발점으로 사용하세요.`
  }
...
옵션 반영 가이드 ⚠️ 반드시 적용:
- 위 "추가 옵션"에 적힌 항목(자유 텍스트)을 **하나하나 읽고**, 한국 중고차 시장에서 값을 올리는 옵션은 반드시 현재가에 반영하세요. 옵션을 무시하지 마세요.
- 대표 가산 옵션(동급 깡통 대비 가산폭 예시):
  · 파노라마 선루프/선루프: +1~3%
  · 가죽시트(특히 나파)·통풍/열선시트: +1~2%
  · 헤드업 디스플레이(HUD): +0.5~1.5%
  · 어댑티브 크루즈/차선유지 등 ADAS 풀패키지: +1~3%
  · 어라운드뷰/서라운드뷰 카메라: +0.5~1.5%
  · 프리미엄 사운드(B&O·메르디안·JBL 등): +0.5~1.5%
  · 4WD/AWD·디지털 키·전동트렁크·뒷좌석 통풍 등: 차종 맞춰 +0.5~2%
  · 수입차 개별 옵션(예: 포르쉐 PASM·스포츠크로노 등)은 수백만원 단위로 시세를 가르므로 더 크게 반영.
- 옵션이 많으면 합산 가산은 최대 +6~8% 선에서 cap (이중 가산 방지).
- **색상도 시세 민감 요인**: 검정·흰색·회색·은색은 표준(0%), 빨강·노랑·파랑·녹색 등 호불호 색은 -1~3%, 반대로 인기 특별색(예: 일부 수입차의 화이트펄·매트)은 +0~1%. 내장도 베이지/투톤 오염 부각 시 -0~2%.
- rationale [시세 평가] 단락에 "어떤 옵션을 얼마나 반영했는지" 한 줄로 언급하세요.
...
[장기 예측 절대 하한 — 절대 원칙 ⚠️ 중요]
한국 중고차 시장에서 멀쩡한 차량(폐차 수준 아님)이 단기간에 0원이나 헐값으로 떨어지는 일은 없습니다. AI 가 너무 비관적으로 예측하지 않도록 아래 하한을 반드시 지키세요:

- predicted_1m >= current_price × 0.95 (1개월에 5% 이상 떨어지면 비정상)
- predicted_3m >= current_price × 0.90
- predicted_6m >= current_price × 0.85
- predicted_1y >= current_price × 0.78 (1년에 22% 이상 떨어지지 않음)
- predicted_2y >= current_price × 0.65
- predicted_3y >= current_price × 0.55 (3년에 45% 이상 떨어지지 않음)

위 하한은 "차령이 어떻든, 모델이 무엇이든" 적용되는 절대 하한입니다. 5년차 K8 노블레스도 신차 대비 -50% 정도이지 0원이 아닙니다.

[예외 — 위 하한이 적용 안 되는 경우]
- 차량이 이미 8년 초과: 시작점이 이미 낮으므로 추가 감가 폭이 작음 (predicted_3y >= current_price × 0.65 적용)
- 단종 + 부품수급 어려운 모델: predicted_3y >= current_price × 0.45 까지 허용
- 그 외의 경우 위 절대 하한 무조건 준수

[predicted_* sanity check]
산정 후 자문: 모든 예측치가 위 하한을 지키는가? 그래프로 그렸을 때 갑자기 절벽처럼 떨어지는 구간이 있는가? 있으면 부드럽게 조정.
