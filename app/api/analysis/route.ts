...
export async function POST(request: Request) {
  ...
  const { data: vehicle, error: vehicleError } = await supabase
    .from("vehicles")
    .select(
      "id, manufacturer, model, trim, year, mileage, purchase_price, fuel_type, transmission, displacement_cc, body_type, vehicle_class, options, damage_map, wheel_scuff, key_count, plate_number, color, interior_color, registered_at, vin, engine_code, seating_capacity, status, loan_principal, loan_started_at, loan_months, loan_apr",
    )
