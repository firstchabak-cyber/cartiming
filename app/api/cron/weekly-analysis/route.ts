...
export async function GET(request: Request) {
  ...
  const { data: vehicles, error } = await admin
    .from("vehicles")
    .select(
      "id, user_id, manufacturer, model, trim, year, mileage, fuel_type, transmission, displacement_cc, body_type, vehicle_class, options, damage_map, loan_principal, loan_started_at, loan_months, loan_apr",
    );
