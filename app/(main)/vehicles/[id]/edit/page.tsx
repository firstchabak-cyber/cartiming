...
export default async function VehicleEditPage({
  ...
  const { data: vehicle } = await supabase
    .from("vehicles")
    .select(
      "id, manufacturer, model, trim, year, mileage, purchase_price, registered_at, fuel_type, transmission, color, interior_color, plate_number, vin, displacement_cc, body_type, vehicle_class, engine_code, inspection_valid_until, seating_capacity, key_count, wheel_scuff, damage_map, options, loan_principal, loan_started_at, loan_months, loan_apr",
    )
    .eq("id", params.id)
    .eq("user_id", user.id)
    .maybeSingle();
